...
func _enter_tree():
	# Create the clear user data button
	button = Button.new()
...
func show_confirmation_dialog():
	...
	open_dir_btn.icon = get_editor_interface().get_base_control().get_theme_icon("Folder", "EditorIcons")
	open_dir_btn.tooltip_text = "Open user:// directory in file explorer"
	open_dir_btn.pressed.connect(func():
		var user_path = ProjectSettings.globalize_path("user://")
		OS.shell_show_in_file_manager(user_path)
		print("Opening user data directory: ", user_path)
	)
